...
async fn main() {
    ...
    let client_id = std::env::var("CLIENTID").unwrap_or_else(|_| "kimne78kx3ncx6brgo4mv6wki5h1ko".to_string());
    ...
    let pkg: Value = serde_json::from_str(&std::fs::read_to_string("package.json").unwrap_or_default())
...
async fn chat_socket(stream: axum::extract::ws::WebSocket) {
    ...
    let Some(Ok(Message::Text(cmd))) = first else { return; };
    ...
    let Ok((twitch_ws, _)) = tws else { return; };
    ...
    for cap in [
        ...
        let _ = tw_send.send(tokio_tungstenite::tungstenite::Message::Text(cap.into())).await;
    }
    let _ = tw_send
        .send(tokio_tungstenite::tungstenite::Message::Text(format!("JOIN #{channel}").into()))
        .await;
...
async fn gql(state: &AppState, body: Value, mobile: bool) -> Result<Value, ()> {
    ...
    headers.insert("Client-ID", HeaderValue::from_str(&state.client_id).unwrap());
    headers.insert(header::USER_AGENT, HeaderValue::from_str(&state.user_agent).unwrap());
    headers.insert(header::REFERER, HeaderValue::from_static(if mobile {"https://m.twitch.tv/"} else {"https://www.twitch.tv/"}));
    headers.insert(header::ORIGIN, HeaderValue::from_static(if mobile {"https://m.twitch.tv/"} else {"https://www.twitch.tv/"}));

    let req = state.client.post("https://gql.twitch.tv/gql").headers(headers).json(&body).send().await.map_err(|_| ())?;
    if req.status() != StatusCode::OK { return Err(()); }
    req.json().await.map_err(|_| ())
}

fn invalid() -> Json<Value> { Json(json!({"invalid":true})) }

async fn stream_info(State(state): State<Arc<AppState>>, Path(username): Path<String>) -> impl IntoResponse {
    ...
    let (Ok(cat), Ok(avatar), Ok(title)) = (cat, avatar, title) else { return invalid().into_response(); };

    let user_id = cat.pointer("/data/user/id").and_then(|v| v.as_str()).unwrap_or("");
    let game = cat.pointer("/data/user/stream/game/name").and_then(|v| v.as_str()).unwrap_or("");
    let av = avatar.pointer("/data/userOrError/profileImageURL").and_then(|v| v.as_str()).unwrap_or("");
    let t = title.pointer("/data/user/broadcastSettings/title").and_then(|v| v.as_str()).unwrap_or("");
    if user_id.is_empty() || game.is_empty() || av.is_empty() || t.is_empty() { return invalid().into_response(); }

    let views = gql(&state, json!({"query": format!("query UseViewCount {{ user(id: {}) {{ stream {{ viewersCount }} }} }}", user_id),"variables":{}}), false).await;
    let Ok(views) = views else { return invalid().into_response(); };
    let v = views.pointer("/data/user/stream/viewersCount").and_then(|x| x.as_i64()).unwrap_or(-1);
    if v < 0 { return invalid().into_response(); }

    Json(json!({"views":v,"game":game,"avatar":av,"title":t})).into_response()
}

async fn streamer_info(State(state): State<Arc<AppState>>, Path(username): Path<String>) -> impl IntoResponse {
    let u = username.to_lowercase();
    ...
    let (Ok(shell), Ok(home)) = (shell, home) else { return invalid().into_response(); };
    let data = json!({
...
async fn vod_info(State(state): State<Arc<AppState>>, Path(id): Path<String>) -> impl IntoResponse {
    ...
    let (Ok(meta), Ok(name)) = (meta, name) else { return invalid().into_response(); };
    let login = name.pointer("/data/video/owner/login").and_then(|v| v.as_str()).unwrap_or("");
    if login.is_empty() { return invalid().into_response(); }
    ...
    let Ok(avatar) = avatar else { return invalid().into_response(); };
...
async fn vod_comments(State(state): State<Arc<AppState>>, Path((id, offset)): Path<(String, String)>) -> impl IntoResponse {
    ...
    let Ok(res) = res else { return invalid().into_response(); };
    let edges = res.pointer("/data/video/comments/edges").and_then(|v| v.as_array()).cloned().unwrap_or_default();
    let data: Vec<Value> = edges.into_iter().filter_map(|e| {
        let n = e.get("node")?;
    ...
    ([(header::CACHE_CONTROL, "max-age=3600")], Json(json!({"valid":true,"data":data}))).into_response()
}

async fn vod_list(State(state): State<Arc<AppState>>, Path((username, filter, limit)): Path<(String, String, usize)>) -> impl IntoResponse {
    let broadcast_type = if filter == "ALL" { Value::Null } else { json!(filter) };
    ...
    let Ok(res) = res else { return invalid().into_response(); };
    let edges = res.pointer("/data/user/videos/edges").and_then(|v| v.as_array()).cloned().unwrap_or_default();
    ...
    ([(header::CACHE_CONTROL, "max-age=1800")], Json(json!({"vods": vods}))).into_response()
}

async fn clip_info(State(state): State<Arc<AppState>>, Path((username, id)): Path<(String, String)>) -> impl IntoResponse {
    ...
    let (Ok(metadata), Ok(media)) = (metadata, media) else { return invalid().into_response(); };
    let m = to_clip_media(&media);
    if m.is_empty() { return invalid().into_response(); }
    ([(header::CACHE_CONTROL, "max-age=3600")], Json(json!({
      "metadata": {
...
fn to_clip_media(media: &Value) -> Vec<Value> {
    let token_sig = media.pointer("/data/clip/playbackAccessToken/signature").and_then(|v| v.as_str()).unwrap_or("");
    let token_val = media.pointer("/data/clip/playbackAccessToken/value").and_then(|v| v.as_str()).unwrap_or("");
...
async fn clips_list(State(state): State<Arc<AppState>>, Path((username, filter, limit)): Path<(String, String, usize)>) -> impl IntoResponse {
    ...
    let Ok(res) = res else { return invalid().into_response(); };
    let edges = res.pointer("/data/user/clips/edges").and_then(|v| v.as_array()).cloned().unwrap_or_default();
...
async fn user_info(State(state): State<Arc<AppState>>, Path(username): Path<String>) -> impl IntoResponse {
    let u = username.to_lowercase();
    ...
    let (Ok(about), Ok(stream_meta), Ok(shell)) = (about, stream_meta, shell) else {
        return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error":{"status":500,"message":"fetch failed"},"data":null}))).into_response();
    };
...
async fn emotes(State(state): State<Arc<AppState>>, Path(username): Path<String>) -> impl IntoResponse {
    ...
    let Ok(user) = user else { return (StatusCode::BAD_REQUEST, Json(json!({"status":400,"message":"Invalid user"}))).into_response(); };
    let uid = user.pointer("/data/user/id").and_then(|v| v.as_str()).unwrap_or("");
    if uid.is_empty() { return (StatusCode::BAD_REQUEST, Json(json!({"status":400,"message":"Invalid user"}))).into_response(); }
    ...
    let Ok(list) = list else { return (StatusCode::BAD_REQUEST, Json(json!({"status":400,"message":"Invalid user"}))).into_response(); };

    let mut out = vec![];
    for e in list.pointer("/data/channel/localEmoteSets").and_then(|v| v.as_array()).cloned().unwrap_or_default() {
        for em in e.get("emotes").and_then(|v| v.as_array()).cloned().unwrap_or_default() {
    ...
    ([(header::CACHE_CONTROL, "max-age=3600, public")], Json(json!({"data":out}))).into_response()
}

#[derive(Deserialize)]
struct QualityQuery { quality: Option<u32> }

async fn stream_proxy(State(state): State<Arc<AppState>>, Path(username): Path<String>, Query(query): Query<QualityQuery>) -> impl IntoResponse {
    ...
    let token_req = state.client.post("https://gql.twitch.tv/gql")
      ...
      .send().await;
    let Ok(token_req) = token_req else { return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error":true,"data":null}))).into_response(); };
    let tr: Value = token_req.json().await.unwrap_or_default();
    let sig = tr.pointer("/data/user/stream/playbackAccessToken/signature").and_then(|v| v.as_str()).unwrap_or("");
    let token = tr.pointer("/data/user/stream/playbackAccessToken/value").and_then(|v| v.as_str()).unwrap_or("");
    if sig.is_empty() || token.is_empty() { return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error":true,"data":null}))).into_response(); }
...
async fn vod_proxy(State(state): State<Arc<AppState>>, Path(id): Path<String>, Query(query): Query<QualityQuery>) -> impl IntoResponse {
    ...
    let Ok(token) = token else { return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error":true,"data":null}))).into_response(); };
    let sig = token.pointer("/data/videoPlaybackAccessToken/signature").and_then(|v| v.as_str()).unwrap_or("");
    let val = token.pointer("/data/videoPlaybackAccessToken/value").and_then(|v| v.as_str()).unwrap_or("");
    if sig.is_empty() { return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error":true,"data":null}))).into_response(); }
    ...
    let list_text = match fetch_raw_text(&state, &playlist_url, false).await { Ok(t) => t, Err(r) => return r };
    let selected = select_playlist(&list_text, quality).unwrap_or_default();
    if selected.is_empty() { return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error":true,"data":null}))).into_response(); }
    let manifest = match fetch_raw_text(&state, &selected, false).await { Ok(t) => t, Err(r) => return r };
    let base = selected.split("index-dvr.m3u8").next().unwrap_or("");
    let body = manifest.lines().map(|line| {
        if line.starts_with('#') || line.trim().is_empty() { line.to_string() } else { format!("/api/proxy?url={}", STANDARD.encode(format!("{base}{line}"))) }
    }).collect::<Vec<_>>().join("\n");
...
fn select_playlist(manifest: &str, quality: u32) -> Option<String> {
    ...
    for i in 0..lines.len().saturating_sub(1) {
        if lines[i].contains("RESOLUTION=") && lines[i].contains(&format!("x{quality}")) {
            ...
            if next.starts_with("http") { return Some(next.to_string()); }
        }
    }
    lines.iter().find(|l| l.starts_with("http")).map(|x| x.to_string())
}

async fn fetch_text(state: &AppState, url: &str, mobile: bool) -> Response {
    match fetch_raw_text(state, url, mobile).await {
        Ok(text) => ([(header::CONTENT_TYPE, "application/vnd.apple.mpegurl")], text).into_response(),
        Err(r) => r,
    }
}
...
async fn fetch_raw_text(state: &AppState, url: &str, mobile: bool) -> Result<String, Response> {
    let req = state.client.get(url)
        .header(header::USER_AGENT, &state.user_agent)
        .header(header::REFERER, if mobile {"https://m.twitch.tv"} else {"https://player.twitch.tv"})
        .header(header::ORIGIN, if mobile {"https://m.twitch.tv"} else {"https://player.twitch.tv"})
        .header("Client-ID", &state.client_id)
        .send().await;
    let Ok(resp) = req else { return Err((StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error":true,"data":null}))).into_response()); };
    if resp.status() != StatusCode::OK { return Err((StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error":true,"data":null}))).into_response()); }
    let t = resp.text().await.unwrap_or_default();
    Ok(t)
}

#[derive(Deserialize)]
struct UrlQ { url: Option<String> }

async fn urlproxy(State(state): State<Arc<AppState>>, Query(q): Query<UrlQ>) -> impl IntoResponse {
    let Some(url) = q.url else { return (StatusCode::BAD_REQUEST, Json(json!({"invalid":true}))).into_response() };
    pipe_url(&state, &url, "https://player.twitch.tv", "https://player.twitch.tv").await
}

async fn clip_proxy(State(state): State<Arc<AppState>>, Path((media, sig, token)): Path<(String, String, String)>) -> impl IntoResponse {
    let url = format!("{}?sig={}&token={}", urlencoding::decode(&media).unwrap_or_default(), sig, urlencoding::encode(&token));
    pipe_url(&state, &url, "https://player.twitch.tv", "https://player.twitch.tv").await
}

async fn proxy(State(state): State<Arc<AppState>>, Query(q): Query<UrlQ>) -> impl IntoResponse {
    let Some(encoded) = q.url else { return (StatusCode::BAD_REQUEST, "No url provided.").into_response(); };
    ...
    pipe_url(&state, &decoded, "https://www.twitch.tv", "https://www.twitch.tv").await
}
...
async fn pipe_url(state: &AppState, url: &str, referer: &str, origin: &str) -> Response {
    let resp = state.client.get(url)
        ...
        .header("Client-ID", &state.client_id)
        .send().await;
    let Ok(resp) = resp else { return (StatusCode::BAD_REQUEST, "err").into_response(); };
    if resp.status() != StatusCode::OK { return (StatusCode::BAD_REQUEST, Json(json!({"invalid":true}))).into_response(); }
    let status = resp.status();
    let ct = resp.headers().get(header::CONTENT_TYPE).cloned().unwrap_or(HeaderValue::from_static("application/octet-stream"));
    let cc = resp.headers().get(header::CACHE_CONTROL).cloned();
    let bytes = resp.bytes().await.unwrap_or_default();

    let mut out_headers = HeaderMap::new();
    out_headers.insert(header::CONTENT_TYPE, ct);
    if let Some(ccv) = cc { out_headers.insert(header::CACHE_CONTROL, ccv); }
    (status, out_headers, bytes).into_response()
}

#[derive(Deserialize)]
struct ClipPageQuery { quality: Option<String>, embed: Option<String> }
...
async fn clip_page_or_index(
    ...
    let metadata = gql(&state, json!({"operationName":"ClipMetadata","variables":{"channelLogin":username.to_lowercase(),"clipSlug":id},"extensions":{"persistedQuery":{"version":1,"sha256Hash":"ab70572e66f164789c87936a8291fd15e29adc2cea0114b02e60f17d60d6d154"}}}), false).await;
    let media = gql(&state, json!({"operationName":"VideoAccessToken_Clip","variables":{"slug":id},"extensions":{"persistedQuery":{"version":1,"sha256Hash":"36b89d2507fce29e5ca551df756d27c1cfe079e2609642b4390aa4c35796eb11"}}}), false).await;
    let (Ok(metadata), Ok(media)) = (metadata, media) else { return Html(render_clip_invalid(&state.version)).into_response(); };
    let media_vec = to_clip_media(&media);
    if media_vec.is_empty() {
        return Html(render_clip_invalid(&state.version)).into_response();
    }
    let chosen = query.quality.as_deref().and_then(|q| media_vec.iter().find(|m| m.get("quality").and_then(|v| v.as_str()) == Some(q))).cloned().unwrap_or_else(|| media_vec[0].clone());
...
fn render_clip_invalid(version: &str) -> String {
    format!("<!doctype html><html><head><title>Twineo - Clip</title><link rel=\"stylesheet\" href=\"/styles.min.css\"></head><body><div class=\"container\"><h1>Not found</h1><p>Clip not found.</p></div><footer><p>Twineo Version {version}</p></footer></body></html>")
}

fn render_clip_page(state: &AppState, username: &str, _slug: &str, src: &str, metadata: &Value) -> String {
    let title = metadata.pointer("/data/clip/title").and_then(|v| v.as_str()).unwrap_or("");
    let game = metadata.pointer("/data/clip/game/displayName").and_then(|v| v.as_str()).unwrap_or("");
    let author = metadata.pointer("/data/clip/curator/displayName").and_then(|v| v.as_str()).unwrap_or("");
    let views = metadata.pointer("/data/clip/viewCount").and_then(|v| v.as_i64()).unwrap_or(0);
    let date = metadata.pointer("/data/clip/createdAt").and_then(|v| v.as_str()).unwrap_or("");
    let avatar = metadata.pointer("/data/user/profileImageURL").and_then(|v| v.as_str()).unwrap_or("");
    ...
    format!("<!doctype html><html><head><meta charset=\"UTF-8\" /><title>Twineo - Clip {title}</title>{video_tags}<link rel=\"stylesheet\" href=\"/styles.min.css\"><link rel=\"stylesheet\" href=\"/poppins.css\"></head><body><div class=\"container\"><video controls src=\"{src}\"></video><span id=\"date\"></span><h3>{title}</h3><div>{game}</div><div><span>By {author}</span> <span>{views} views</span></div><div><a href=\"/{username}?home=true\"><img class=\"w-8 rounded-full\" src=\"/api/urlproxy?url={avatar}\" /></a><a href=\"/{username}?home=true\">{username}</a></div></div><script>const date=Date.parse('{date}')-Date.now(),sec=Math.abs(Math.floor(date/1000)),min=Math.abs(Math.floor(sec/60)),hours=Math.abs(Math.floor(min/60)),days=Math.abs(Math.floor(hours/24));document.getElementById('date').innerText=`${{days}} days, ${{hours%24}} hours, ${{min%60}} minutes, and ${{sec%60}} seconds ago`;</script><footer><p>Twineo Version {} - <a href=\"https://codeberg.org/CloudyyUw/twineo\">Source</a></p></footer></body></html>", state.version)
}
